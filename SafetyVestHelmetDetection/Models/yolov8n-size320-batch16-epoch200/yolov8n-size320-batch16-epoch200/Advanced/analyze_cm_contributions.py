...
def load_project_imdb(project_path: str) -> dict:
    ...
    session_map = {}  # session_id -> {path, name, dataset}
...
def generate_markdown_report(contributions: dict, output_path: str, session_map: dict):
    ...
    Args:
        contributions: The contributions JSON data
        output_path: Path to save the Markdown report
        session_map: Mapping of session_id to image path info from project file
    """
    ...
    # Detailed section for each cell
    for cell_key, cell_data in sorted(contrib_data.items()):
        pred_name = cell_data.get("predicted_class_name", "?")
        true_name = cell_data.get("true_class_name", "?")
        ...
        # Determine if this is TP, FP, FN, or TN
        ...
        # Group by session - only include sessions found in project
        matched_sessions = {}
        unmatched_count = 0
        ...
        # Table with clickable image links - only for matched sessions
        ...
        # Expandable section with all matched image paths
...
def main():
    ...
    # Load project and build session mapping
